...
async function seedBeiteomrader() {
    try {
        ...
        console.log('Cleared existing beiteområder');

        for (const data of mockBeiteomrader) {
            const beiteomrade = new Beiteomrade(data);
            await beiteomrade.save();
            console.log(`Created beiteområde: ${beiteomrade.navn}`);
        }

        console.log('Finished seeding beiteområder');
        process.exit(0);
    } catch (error) {
        console.error('Error seeding beiteområder:', error);
        process.exit(1);
    }
}
